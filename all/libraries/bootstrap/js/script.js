    ...
    function sendNotify(){
            ...
            var addressError = $('<span id="address_span" class="help-inline">Select an address on map</span>');
     
            var notifyJSON = JSON.stringify(notifyObj);
            console.log(notifyJSON);
     
            if( (notifyType.type != "Select type") && (notifyType.subtype != "Select subtype") && (notifyObj.lat) ){
                            alert("Pota");
                            $.ajax({
                                    url: url,
                                    type: 'POST',
            ...
            else if( (notifyObj.type == "Select type") && $("#notifyType:not(:has('span'))") ){    
                    $('#notifyType').parent().addClass("error");
                    $('#notifyType').after(typeError);
            }
            else if( (notifyObj.subType == "Select subtype") && $("#notifySubType:not(:has('span'))") ){
                    $('#notifySubType').parent().addClass("error");
                    $('#notifySubType').after(subTypeError);
            }
            else if(!(notifyObj.address)){
                    $('#notifyAddress').parent().addClass("error");
                    $('#notifyAddress').after(addressError);
            }
     
            $('#notifyType').on("keypress", function(){
                    $('#notifyType').parent().removeClass("error");
                    $('#type_span').fadeOut();
                    $('#type_span').remove();
            });
            $('#notifySubType').on("keypress", function(){
                    $('#notifySubType').parent().removeClass("error");
                    $('#subtype_span').fadeOut();
                    $('#subtype_span').remove();
            });
            $('#notifyAddress').on("keypress", function(){
                    $('#notifyAddress').parent().removeClass("error");
                    $('#address_span').fadeOut();
                    $('#address_span').remove();
            });
    }
